...
public:
  ...
  // 방어자의 HP에서 데미지만큼 감소
  static CombatResult attack(CombatStatus &attacker, CombatStatus &defender);

  // 두 객체가 전투 가능한지 확인 (둘 다 HP를 가지고 있는지)
  static bool canCombat(const CombatStatus &entity1, const CombatStatus &entity2);

  // 엔티티가 살아있는지 확인 (HP > 0)
  static bool isAlive(const CombatStatus &entity);
